#!/usr/bin/env Rscript
# EH-BAM version of short_somatic_perIndividual.R
# Differences from the original:
#   1. Accepts a third argument OUT_DIR; reads/writes files from/to that directory.
#   2. Input and output filenames include GENE and REP (extracted from DAT) to avoid
#      collisions when multiple loci are processed in parallel.
#   3. rateSTRT_mis NaN fix: ExpansionHunter re-encodes base quality as binary
#      (I = PHRED 40, ! = PHRED 0).  When all flanking bases are marked '!' the
#      mismatch-rate check produces NaN (mean of empty vector).  In R NaN < 0.05
#      evaluates to FALSE, incorrectly rejecting every read.  The fix treats
#      NA/NaN as "no evidence of mismatch" (pass-through) using is.na().
#   4. Consensus quality filter (%in% c("I")) is already correct for EH binary
#      quality encoding and is left unchanged.
options(echo=FALSE);
...
# Extract locus components from DAT
parts   = stringr::str_split(DAT,"_")[[1]]
GENE    = parts[1]
REP     = parts[7]
...
STARTstr = parts[5]
ENDstr   = parts[6]
repLen   = stringr::str_length(parts[7])
L_FLANK  = as.numeric(stringr::str_split(stringr::str_remove(parts[8],"L"),",")[[1]])
R_FLANK  = as.numeric(stringr::str_split(stringr::str_remove(parts[9],"R"),",")[[1]])

# Load individual read data written by somatic_on_EH_bams.sh
...
cat("DEBUG: Command line args - ID:", ID, "DAT:", DAT, "OUT_DIR:", OUT_DIR, "\n")
...
consensus = function(SEQUENCES,QUALITIES,START){
  if(length(SEQUENCES) == 1){ return(SEQUENCES) }
    ...
    for(i in 1:nrow(SEQdf)){
      # EH re-encodes quality as binary: I = PHRED 40 (trusted), ! = PHRED 0 (untrusted).
      # Filtering for 'I' selects only trusted bases, consistent with the EH encoding.
...
mismatch_bases <- function(strand,jump,som_seq,som_qual,seq,start,end,L_flank_set,R_flank_set,repeatLength){
  ...
  # (2) # of base pairs checked. For this check restrict to reasonable-quality bases (QUAL>=25).
  # EH note: EH marks flanking bases as '!' (PHRED 0) when uncertain about placement.
  # If all flanking bases are '!', compar_qual_values >= 25 is all FALSE, the filtered
  # subset is empty, and mean() returns NaN.  NaN < 0.05 is FALSE in R, which would
  # incorrectly reject the read.  We return NaN here and handle it with is.na() in the
  # somatic filter below (NaN is caught by is.na() in R).
  if(strand == "reverse"){ 
...
tab_sum = tab_sum_temp %>% ungroup() %>% rowwise() %>% 
  ...
  mutate(j_neg2 =  mismatch_bases(strand=as.character(V2),jump=-2,
         ...
         j_pos2 =  mismatch_bases(strand=as.character(V2),jump=2,
                                  ...
                                  L_flank_set=L_FLANK,R_flank_set=R_FLANK,repeatLength=repLen)) %>%
  rowwise() %>% mutate(
    # EH fix: rateSTRT_mis can be NaN when EH marks all flanking bases as '!' (PHRED 0),
    # leaving no qualifying bases for the mismatch rate calculation.  In R, NaN < 0.05
    # evaluates to FALSE, which would incorrectly reject the read.  is.na() catches both
    # NA and NaN, so (is.na(x) | x < 0.05) treats "unchecked" as "no evidence of mismatch".
    somatic_neg2 = ifelse(j_neg2$LEN >= 4 & j_neg2$NONF_SCORE < 0.2 & (is.na(j_neg2$rateSTRT_mis) | j_neg2$rateSTRT_mis < 0.05) & 
                             ((j_neg2$flnk_l_hiP_sub == 1 | is.na(j_neg2$flnk_l_hiP_sub)) & 
                                (j_neg2$flnk_r_hiP_sub == 1 | is.na(j_neg2$flnk_r_hiP_sub))),1,0),
    somatic_neg1 = ifelse(j_neg1$LEN >= 4 & j_neg1$NONF_SCORE < 0.2 & (is.na(j_neg1$rateSTRT_mis) | j_neg1$rateSTRT_mis < 0.05) & 
                            ((j_neg1$flnk_l_hiP_sub == 1 | is.na(j_neg1$flnk_l_hiP_sub)) & 
                               (j_neg1$flnk_r_hiP_sub == 1 | is.na(j_neg1$flnk_r_hiP_sub))),1,0),
    somatic_pos1 = ifelse(j_pos1$LEN >= 4 & j_pos1$NONF_SCORE < 0.2 & (is.na(j_pos1$rateSTRT_mis) | j_pos1$rateSTRT_mis < 0.05) & 
                            ((j_pos1$flnk_l_hiP_sub == 1 | is.na(j_pos1$flnk_l_hiP_sub)) & 
                               (j_pos1$flnk_r_hiP_sub == 1 | is.na(j_pos1$flnk_r_hiP_sub))),1,0),
    somatic_pos2 = ifelse(j_pos2$LEN >= 4 & j_pos2$NONF_SCORE < 0.2 & (is.na(j_pos2$rateSTRT_mis) | j_pos2$rateSTRT_mis < 0.05) & 
